...
// 5 --- 绑定 this 的方法
{
    ...
    {
        // 函数实例的 call ，可以指定函数内部 this 的指向(即函数执行时所在的作用域)，然后在指定的作用域中，调用该函数
        var obj = {};
        var f = function () {
            return this;
        };
        console.log('[call]---', f() === obj); // false
        console.log('[call]---', f.call(obj) === obj); // true

        //上面代码中，全局环境运行函数f时， this 指向全局环境(浏览器为 window 对象)
        // call 方法可以改变this的指向，指定this指向对象obj，然后在对象obj的作用域中运行函数f
        // call 方法的参数，应该是一个对象
        // 如果参数为 空 、 null 、 undefined ，则默认传入全局对象
        // 如果参数是一个原始值，那么这个原始值会自动转成对应的包装对象，然后传入 call 方法
        console.log('[call]---', f.call(5)); // [Number: 5]
        // call 方法还可以接受多个参数
        {
            // func.call(thisValue, arg_1, arg_2, ...)
            // thisValue ： this 所要指向的那个对象
            // 后面的参数则是函数调用时所需的参数
            function add(a, b) {
                return a + b;
            }
            console.log('[call]---', add.call(this, 1, 2)); // 3
        }
        // call 方法的一个应用是调用对象的原生方法
        {
            var obj = {};
            console.log('[call]---', obj.hasOwnProperty('toString')); // false

            // 覆盖掉继承的hasOwnProperty方法
            obj.hasOwnProperty = function () {
                return true;
            };
            console.log('[call]---', obj.hasOwnProperty('toString')); // true
            console.log('[call]---', Object.prototype.hasOwnProperty.call(obj, 'toString')); // false
            // 上面代码中， hasOwnProperty 是 obj 对象继承的方法，如果这个方法一旦被覆盖，就不会得到正确的结果
            // call 方法可以解决这个问题，它将 hasOwnProperty 方法的原始定义放到 obj 对象上执行，这样无论 obj 上有没有同名方法，都不会影响结果
        }
    }
    // 5.2 --- Function.prototype.apply()
    {
        // apply 方法的作用与 call 方法类似，也是改变 this 指向，然后调用该函数
        // 唯一的区别是，它接受一个数组作为函数执行时的参数，使用格式如下：
        // func.apply(thisValue, args | arguments)
        // 参数 func ：函数指针
        // 参数 thisValue (Object) ： this 所要指向的那个对象；如果设为 null 或 undefined ，则等同于指定全局对象
        // 参数 args | arguments (Array | arguments 对象) ：参数数组或 arguments 对象，所有成员依次作为参数，传入原函数
        // 原函数的参数，在 call 方法中必须一个个添加，但是在 apply 方法中，必须以数组形式添加
        function printApplyArgs(param) {
            console.log('[apply]---param', param);
            console.log('[apply]---arguments', arguments);
        }
        printApplyArgs();
        // undefined
        // [Arguments] {}
        printApplyArgs(undefined);
        // undefined
        // { '0': undefined }
        printApplyArgs(null);
        // null
        ...
        function printAdd(x, y) {
            console.log('[apply]---printAdd', x + y);
        }
        printAdd.call(null, 1, 1); // 2
        printAdd.apply(null, [1, 1]); // 2
        // 上面代码中， printAdd 函数本来接受 2 个参数，使用 apply 方法以后，就变成可以接受一个数组作为参数
        // 利用这一点，可以做一些有趣的应用
        {
            // 应用---1---找出数组最大元素
            {
                var a = [10, 2, 4, 15, 9];
                console.log('[apply]---找出数组最大元素', Math.max.apply(null, a)); // 15
            }
            // 应用---2---将数组的空元素变为 undefined
            {
                var a = ['a', , 'b'];
                console.log('[apply]---将数组的空元素变为undefined', a);
                // [ 'a', <1 empty item>, 'b' ]
                console.log('[apply]---将数组的空元素变为undefined', Array.apply(null, a));
                // [ 'a', undefined, 'b' ]
                // 空元素与 undefined 的差别在于，数组的 forEach 方法会跳过空元素，但是不会跳过 undefined
                function print(value) {
                    console.log('[apply]---将数组的空元素变为undefined', value);
                }
                a.forEach(print);
                // a
                // b
            ...
            // 应用---3---转换类似数组的对象
            {
                ...
                console.log('[apply]---转换类似数组的对象', Array.prototype.slice.apply({
                    0: 1,
                    length: 1
                })); // [ 1 ]
                console.log('[apply]---转换类似数组的对象', Array.prototype.slice.apply({
                ...
                //上 面代码的apply方法的参数都是对象，但是返回结果都是数组
                // 这个方法起作用的前提是：被处理对象必须有 length 属性，以及相对应的数字键
            }
            // 应用---4---绑定回调函数的对象
            {
                var o = new Object();
