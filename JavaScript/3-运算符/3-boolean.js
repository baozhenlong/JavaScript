...
// 2 --- 逻辑非，取反运算符
{
    ...
    // 如果对一个值连续做 2 次取反运算，等于将其转为对应的布尔值，与 Boolean 函数的作用相同；这是一种常用的类型转换写法
    console.log('[取反运算符]---', !!{}); // true
    console.log('[取反运算符]---', Boolean({})); // true
    // 2 次取反：将一个值转为布尔值的简便写法
}
...
// 3 --- 逻辑与，且运算符
{
    ...
    var x = 1;
    (1 - 1) && (x++);
    // 第一个运算子的布尔值为 false ，直接返回它的值 0 ，而不再对第二个运算子求值，所以变量 x 的值没变
...
// 4 --- 或运算符
{
    ...
    // 或运算符可以多个连用，这时返回第一个布尔值为 true 的表达式的值；如果所有所有表达式都为 false ，则返回最后一个表达式的值
    console.log('[或运算符]---多个连用 ', false || '' || null || true || 4); //true
    console.log('[或运算符]---多个连用 ', false || '' || 0); //0
    // 或运算符通常为一个变量设置默认值
    var y = x || 2;
    // 如果 x 的布尔值为 false ，则 y 的值设置为 2
}
