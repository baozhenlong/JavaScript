...
//2.1---基本规则
{
    ...
    // 布尔值
    {
        // 后台调用 Number 将布尔值转成数值，然后再相加
        console.log('[加法运算符]---布尔值 true + true = ', true + true); //2
        console.log('[加法运算符]--- 布尔值 1 + true = ', 1 + true); //2
    }
    // 字符串
    {
        // 字符串 + 字符串，这时加法运算符会变成连接运算符，返回一个新的字符串，将 2 个原字符串连接在一起
    ...
    // null 、 undefined
    {
        // 后台调用 Number 将 null 、 undefined 转成数值，然后再相加
        console.log('[加法运算符]---null ', null + 1); // 1
        console.log('[加法运算符]---undefined ', undefined + 1); // NaN
    }
    // 加法运算符是在运行时决定，到底执行相加，还是执行连接
    // 即运算子的不同，导致了不同的语法行为，这种现象称为"重载"
    // 由于加法运算符存在重载，可能执行 2 种运算，使用时必须很小心
...
// 2.2 --- 对象的相加
{
    ...
    //自定义 valueOf 方法或 toString 方法
    {
        ...
        // 上面代码中，由于 valueOf 方法直接返回一个原始类型的值，所以不再调用 toString方法
        var obj = {
            toString: function () {
                return 'hello';
            }
...
// 4 --- 自增和自减运算符
{
    ...
    // 作用：将运算子首先转为数值，然后 +1 或 -1 ，会修改原始变量
    var x = 1;
    ++x;
    ...
    // 自增和自减运算符是仅有的 2 个具有副作用的运算符，其他运算符都不会改变变量的值
    // 注意：
    // 自增自减运算符，放在变量之后，会先返回变量操作前的值，再进行自增/自减操作
...
{
    // +-(param)
    // 参数 param ：如果是非数值，会先对其进行 Number 转换
    // 数值运算符( + )
    {
        // 同样使用加号，但是它是一元运算符(只需要一个操作数)；而加法运算符是二元运算符(需要两个操作数)
        // 数值运算符的作用：可以将任何值转为数值(与 Number 函数的作用相同)
    ...
    // 负数值运算符(-)
    {
        // 同样具有将一个值转为数值的功能，只不过得到的值正负向反
        // 连用 2 个负数值运算符，等同于数值运算符
        var x = 1;
