...
// 1 --- 定义
{
    // 数组是按次序排列的一组值；每个值的位置都有编号（从 0 开始），整个数组用方括号表示
    var arr = ['a', 'b', 'c'];
...
// 2 --- 数组的本质
{
    ...
    // 对象有 2 中读取成员的方法：点结构 (object.key) 和方括号 (object[key])
    // 但对于数值的键名，不能使用点结构
    var arr_2 = [1, 2, 3];
    // arr.0---报错
    // 上面代码中， arr.0 的写法不合法，因为单独的数值不能作为标识符
    // 所以，数组成员只能用方括号 arr[0] 表示（方括号是运算符，可以接受数值）
    // 读取和设置数组
    var arr = ['red', 'green'];
    console.log('[数组的本质]---读取', arr[0]); // red --- 显示第一项
    arr[1] = 'white'; // 修改第二项
    console.log('[数组的本质]---修改', arr[1]); // white
    arr[3] = 'black'; // 新增第 4 项
    console.log('[数组的本质]---空位', arr[2] === undefined); //true
    console.log('[数组的本质]---arr', arr); // [ 'red', 'white', <1 empty item>, 'black' ]
}
...
// 3 --- length 属性
{
    // length 的定义
    {
        // 数组的 length 属性，返回数组的成员数量（ 始终 >= 0 ）
        // JavaScript 使用一个 32 位整数，保存数组的元素个数；这意味着，数组成员最多只有 2^32-1 个；即 length 属性的最大值就是 2^32-1
        // 只要是数组，就一定有 length 属性；该属性是一个动态的值，等于键名中的最大整数 +1
        var arr = ['a', 'b'];
    ...
    // 修改length
    {
        // length 属性是可写的；如果人为设置一个小于 (<) 当前成员个数的值，该数组的成员会自动减少到 length 设置的值
        arr.length = 2;
        console.log('[length属性]--- ', arr); // [ 'a', 'b' ]
        // 清空数组的一个有效方法，就是将 length 属性设为 0
        arr.length = 0;
        console.log('[length属性]---清空数组', arr); // []
        // 如果人为设置 length 大于 (>) 当前元素个数，则数组的成员数量会增加到这个值，新增的位置都是空位
        arr.length = 2;
        console.log('[length属性]---增加项', arr); // [ <2 empty items> ]
        console.log('[length属性]---增加项', arr[0]); // undefined
        //上面代码表示，当 length 属性设为大于 (>) 数组个数时，读取新增的位置都会返回 undefined
    }
    // 如果认为设置 length 为不合法的值， JavaScript 会报错
    {
        // length 不合法 --- 1 --- 设置负值
        try {
            [].length = -1;
        } catch (err) {
            console.log('[length属性]---length 不合法', err); // Invalid array length
        }
        // length 不合法 --- 2 --- 数组元素个数大于等于 (>=) 2^32
        try {
    ...
    // 为数组添加属性
    {
        ...
        // 因为， length 属性的值就是等于最大的数字键 +1 ，而这个数组没有整数键，所以 length 属性保持为 0
        ...
        // 上面代码中，为数组 a 添加了 2 个不合法的数字键，结果 length 属性没有发生变化
...
// 4 --- in 运算符
{
    ...
    console.log('[in运算符]---数字', 2 in arr); // true
    console.log('[in运算符]---字符串', '2' in arr); // true
    console.log('[in运算符]---undefined', 3 in arr); // true
    console.log('[in运算符]---空位', 4 in arr); // false
