...
//1---定义
{
    ...
    //包装对象：分别与数值、字符串、布尔值相对应的Number、String、Boolean三个原生对象
...
//3---原始类型与实例对象的自动转换
{
    ...
    //这时，JavaScript引擎会自动将原始类型的值转为包装对象实例，在使用后立刻销毁实例
    //比如，字符串可以调用length属性，返回字符串的长度
    var str = 'abc';
    console.log('[自动转换]---', str.length);
    //3
    //等同于
    var str_obj = new String(str);
    console.log('[自动转换]---', str_obj);
    //[String: 'abc']
    console.log('[自动转换]---', str_obj.length);
    //3
    str_obj.x = 123;
    //123
    console.log('[自动转换]---', str_obj.x);
    //自动转换成的包装对象是只读的，无法修改，所以字符串无法添加新属性
    str.x = 123;
    console.log('[自动转换]---', str.x);
    //undefined
