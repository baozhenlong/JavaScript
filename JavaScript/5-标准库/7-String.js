...
//4---实例方法
{
    //4.1---String.prototype.charAt()
    {
        ...
        console.log('[实例方法]---charAt', s.charAt(1));
        //'b'
        console.log('[实例方法]---charAt', s.charAt(s.length - 1));
        //'c'
        //这个方法完全可以用数组下标代替
        console.log('[实例方法]---charAt', 'abc' [1]);
        //'b'
        //如果参数为负数，或 >= 字符串的长度，charAt返回空字符串，数组下标返回undefined
        ...
        console.log('[实例方法]---charAt', 'abc' [-1]);
        //undefined
        console.log('[实例方法]---charAt', 'abc' [3]);
        //undefined
    }
    //4.2---String.prototype.charCodeAt()
    {
        //返回字符串指定位置的Unicode码点(十进制表示)，相当于String.fromCharCode()的逆操作
        console.log('[实例方法]---charAt', 'abc'.charCodeAt(1));
        //98
        //如果没有任何参数，则返回首字符的Unicode码点
        console.log('[实例方法]---charAt', 'abc'.charCodeAt());
        //97
        console.log('[实例方法]---charAt', ''.charCodeAt());
        //NaN
        //如果参数为负数，或 >= 字符串的长度，则返回NaN
        console.log('[实例方法]---charAt', 'abc'.charCodeAt(-1));
        //NaN
        console.log('[实例方法]---charAt', 'abc'.charCodeAt(4));
        //NaN
        //注意：
        //charCodeAt方法返回的Unicode码点不会大于0xFFFF(65536)
        //也就是说，只返回2个字节的字符的码点
        //如果遇到码点大于65536的字符(四个字节的字符)，必需连续使用2次charCodeAt，不仅读入charCodeAt(i)，还要读入charCodeAt(i+1)，将2个值放在一起，才能得到准确的字符
    }
    //4.3---String.prototype.concat()
    {
        //用于连接2个字符串，返回一个新字符串，不改变原字符串
        var s_1 = 'abc';
        var s_2 = 'def';
        console.log('[实例方法]---concat', s_1.concat(s_2));
        //'abcdef'
        console.log('[实例方法]---concat', s_1);
        //'abc'
        //如果参数不是字符串，concat方法会将其先转为字符串，然后再连接
        var one = 1;
        var two = 2;
        var three = '3';
        console.log('[实例方法]---concat', ''.concat(one, two, three));
        //'123'
        console.log('[实例方法]---concat', one + two + three);
        //'33'
    }
    //4.4---String.prototype.slice()
    {
        ...
        //第一个参数：表示子字符串的开始位置
        //第二个参数：表示子字符串的结束位置(不含该位置)
        console.log('[实例方法]---slice', 'JavaScript'.slice(0, 4));
        //'Java'
        //如果省略第二个参数，则表示子字符串一直到原字符串结束
        console.log('[实例方法]---slice', 'JavaScript'.slice(4));
        //'Script'
        //如果参数时负值，表示从结尾开始倒数计算的位置，即该负值 + 字符串长度
        console.log('[实例方法]---slice', 'JavaScript'.slice(-6));
        //'Script'
        console.log('[实例方法]---slice', 'JavaScript'.slice(0, -6));
        //'Java'
        console.log('[实例方法]---slice', 'JavaScript'.slice(-2, -1));
        //'p'
        //如果第一个参数 >= 第二个参数，或第一个参数 >= 数组长度，slice方法返回一个空字符串
        console.log('[实例方法]---slice', 'JavaScript'.slice(2, 1));
        //''
    ...
    //4.5---String.prototype.substring()
    {
        ...
        //第一个参数：表示子字符串的开始位置
        //第二个参数：表示子字符串的结束位置(不含该位置)
        console.log('[实例方法]---substring', 'JavaScript'.substring(0, 4));
        //'Java'
        //如果省略第二个参数，则表示子字符串一直到原字符串的结束
        console.log('[实例方法]---substring', 'JavaScript'.substring(4));
        //'Script'
        //如果第一个参数 > 第二个参数，substring方法会自动更换2个参数的位置
        console.log('[实例方法]---substring', 'JavaScript'.substring(10, 4));
        //'Script'
        //等同于
        console.log('[实例方法]---substring', 'JavaScript'.substring(4, 10));
        //'Script'
        //如果参数是负数，substring方法会自动将负数转为0
        console.log('[实例方法]---substring', 'JavaScript'.substring(-3));
        //'JavaScript'
        console.log('[实例方法]---substring', 'JavaScript'.substring(4, -3));
        //'Java'
    ...
    //4.6---String.prototype.substr()
    {
        ...
        //第一个参数：表示子字符串的开始位置(从0开始计算)
        //第二个参数：表示子字符串的长度
        console.log('[实例方法]---substr', 'JavaScript'.substr(4, 6));
        //'Script'
        //如果省略第二个参数，则表示子字符串一直到原字符串的结束
        console.log('[实例方法]---substr', 'JavaScript'.substr(4));
        //'Script'
        //如果第一个参数是负数，表示倒数计算的字符位置
        console.log('[实例方法]---substr', 'JavaScript'.substr(-6));
        //'Script'
    ...
    //4.7---String.prototype.indexOf()，String.prototype.lastIndexOf()
    {
        ...
        console.log('[实例方法]---indexOf', 'hello world'.indexOf('o'));
        //4
        console.log('[实例方法]---indexOf', 'JavaScript'.indexOf('script'));
        //-1
        //indexOf方法还可以接受第二个参数，表示从该位置开始向后匹配
        console.log('[实例方法]---indexOf', 'hello world'.indexOf('o', 4));
        //4
        console.log('[实例方法]---indexOf', 'hello world'.indexOf('o', 6));
        //7
        //lastIndexOf方法的第二个参数表示从该位置起，向前匹配
        console.log('[实例方法]---indexOf', 'hello world'.lastIndexOf('o', 6));
        //4
    }
    ...
    //4.9---String.prototype.toLowerCase()，String.prototype.toUpperCase()
    {
        ...
        //它们都返回一个新字符串，不改变原字符串
        console.log('[实例方法]---toLowerCase', 'Hello World'.toLowerCase());
        //'hello world'
        console.log('[实例方法]---toUpperCase', 'Hello World'.toUpperCase());
        //'HELLO WORLD'
    }
