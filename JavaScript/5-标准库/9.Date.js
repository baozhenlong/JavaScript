...
//2---构造函数的用法
{
    ...
    //如果不加参数，实例代表的就是当前时间
    {
        var today = new Date();
        //Date实例有一个独特的地方：
        //其他对象求值的时候，都是默认调用.valueOf方法
        //但是Date实例求值的时候，默认调用的是toString方法，返回的是一个字符串，代表该实例对应的时间
        console.log('[构造函数]---无参', today);
        //2019-02-11T02:06:55.031Z
        console.log('[构造函数]---无参', today.toString());
    }
    //接受多种格式的参数，返回一个该参数对应的时间实例
    {
        //参数为时间零点开始计算的毫秒数；可以是负整数，代表1970元旦之前的时间
        {
            console.log('[构造函数]---毫秒数', new Date(1378218728000));
            //2013-09-03T14:32:08.000Z
            console.log('[构造函数]---毫秒数', new Date(-1378218728000));
            //1926-04-30T09:27:52.000Z
        }
        //参数为日期字符串；只要是能被Date.parse方法解析的字符串，都可以当参数
        {
            console.log('[构造函数]---日期字符串', new Date('January 6, 2013'));
            //2013-01-05T16:00:00.000Z
            console.log('[构造函数]---日期字符串', new Date('2013-1-6'));
            //2013-01-05T16:00:00.000Z
        }
        //参数为多个整数，代表年、月、日、小时、分钟、秒、毫秒；年月必需，其他参数可选
        {
            console.log('[构造函数]---多个整数', new Date(2013, 0, 6, 1, 2, 3, 4));
            //2013-01-05T17:02:03.004Z
            //各个参数的取值范围：
            //年：使用四位数年份；如果写成2位数或个位数，则加上1900，即10代表1910；如果是负数，表示公元前
...
//3---日期的运算
{
    //类型自动转换时，Date实例
    //如果转为数值，则等于对应的毫秒数
    //2个日期实例对象进行减法运算时，返回的是它们间隔的毫秒数
    //如果转为字符串，则等于对应的日期字符串
    //2个日期实例对象进行加法运算时，返回的是2个字符串连接而成的新字符串
}

//4---静态方法
{
    //时间零点：1970年1月1日00:00:00 UTC
    //4.1---Date.now()
    {
        //返回当前时间距离时间零点的毫秒数
        console.log('[静态方法]---now', Date.now());
        //1549853457076
    }
    //4.2---Date.parse()
    {
    ...
    //4.3---Date.UTC()
    {
        ...
        console.log('[静态方法]---now', Date.UTC(2011, 0, 1, 2, 3, 4, 567));
        // 1293847384567
        //该方法的参数用法与Date构造函数完全一致
        //区别：
        //Date.UTC方法的参数，会被解释为UTC时间(世界标准时间)
        //Date构造函数的参数，会被解释为当前时区的时间
    }
}
...
//5---实例方法
{
    ...
    {
        //返回实例对象距离时间零点对应的毫秒数，该方法等同于getTime方法
        console.log('[实例方法]---valueOf', d.valueOf());
        //1549854772139
        console.log('[实例方法]---getTime', d.getTime());
        //1549854772139
    ...
    //5.2---to类方法
    {
        ...
        //5.2.7---本地时间
        {
            ...
            console.log('[实例方法]---toLocaleString', d.toLocaleString());
            //2019-2-11 11:25:01
            //Date.prototype.toLocaleDateString()：本地日期(不含小时、分、秒)
            console.log('[实例方法]---toLocaleDateString', d.toLocaleDateString());
            //2019-2-11
            //Date.prototype.toLocaleTimeString()：本地时间(不含年、月、日)
            console.log('[实例方法]---toLocaleTimeString', d.toLocaleTimeString());
            //11:25:01
            //这3个方法都有2个可选参数
            //locales：指定所用语言的字符串
            //options：配置对象
    ...
    {
        //getTime()：返回实例对象距离时间零点的毫秒数，等同于valueOf方法
        console.log('[实例方法]---getTime', d.getTime());
        //1549857256284
        //getFullYear()：返回四位的年份
        console.log('[实例方法]---getFullYear', d.getFullYear());
        //2019
        //getMonth()：返回月份；0表示1月，11表示12月
        console.log('[实例方法]---getMonth', d.getMonth());
        //1
        //getDate()：返回实例对象对应每个月的几号；从1开始
        console.log('[实例方法]---getDate', d.getDate());
        //11
        //getHours()：返回小时；0-23
        console.log('[实例方法]---getHours', d.getHours());
        //11
        //getMinutes()：返回分钟；0-59
        console.log('[实例方法]---getMinutes', d.getMinutes());
        //54
        //getSeconds()：返回秒；0-59
        console.log('[实例方法]---getSeconds', d.getSeconds());
        //16
        //getMilliseconds()：返回毫秒；0-999
        console.log('[实例方法]---getMilliseconds', d.getMilliseconds());
        //284
        //getTimezoneOffset()：返回当前时间与UTC的时区差异，以分钟表示，返回结果考虑到了夏令时因素
        //所有这些get方法返回的都是当前时区的时间(整数)
        //Date对象还提供了这些方法对应的UTC版本，用来返回UTC时间
    }
    //5.4---set类方法
    {
        //Date对象提供了一系列set方法，用来设置实例对象的各个方面
        //setFullYear(year [, month, date])：设置4位年份
        ...
        //setTime(ms)：设置毫秒时间戳
        //这些方法基本是跟get方法一一对应，但是没有setDay方法，因为星期几是计算出来的，而不是设置的
        //set方法的参数都会自动折算：
        //如果参数超过当月的最大天数，则向下一个月顺延
        //如果参数是负数，表示从上个月的最后一天开始减去的天数
        //set系列方法除了setTime()，都有对应的UTC版本，即设置UTC时区的时间
    }
}